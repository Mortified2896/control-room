...
async function executionHistory(
  c: PoolClient,
  modelId: string,
  stepId: string,
  tokenEstimate: number,
...
export async function estimateRecommendation(args: {
  recommenderModelId: string;
  providerPath: string;
  promptTokenEstimate: number;
  contextTokenEstimate: number;
  stepId?: string;
}): Promise<RecommendationEstimate> {
  const heuristic = Math.min(
    15_000,
    Math.max(3_000, 2_000 + args.promptTokenEstimate * 3 + args.contextTokenEstimate),
  );
  const hist = await tryDb(
    (c) =>
      recommendationHistory(
        c,
        args.recommenderModelId,
        args.stepId ?? "normal_chat",
        args.promptTokenEstimate,
      ),
    { sampleCount: 0, avgMs: null, p50Ms: null, p99Ms: null, avgIn: null, avgOut: null },
  );
  if (hist.sampleCount >= 100 && hist.p50Ms && hist.p99Ms)
    return {
      expectedLatencyMs: Math.round(hist.p50Ms),
      upperLatencyMs: Math.round(hist.p99Ms),
      estimateQuality: "likely",
      latencyPolicy: "p50_p99_v1",
      latencyBasis: "historical",
      historicalSampleCount: hist.sampleCount,
    };
  if (hist.sampleCount >= 30 && hist.p50Ms && hist.p99Ms)
    return {
      expectedLatencyMs: Math.round((heuristic + hist.p50Ms) / 2),
      upperLatencyMs: Math.round((heuristic * 2.5 + hist.p99Ms) / 2),
      estimateQuality: "uncertain",
      latencyPolicy: "blend_p50_p99_v1",
      latencyBasis: "blended",
      historicalSampleCount: hist.sampleCount,
    };
  return {
    expectedLatencyMs: heuristic,
    upperLatencyMs: Math.round(heuristic * 2.5),
    estimateQuality: hist.sampleCount > 0 ? "uncertain" : "rough",
    latencyPolicy: "heuristic_p50_p99_v1",
    latencyBasis: "heuristic",
    historicalSampleCount: hist.sampleCount,
  };
}

export type ExecutionEstimate = RecommendationEstimate & {
...
export async function estimateExecution(args: {
  ...
  const fallbackOut = Math.min(
    args.maxOutputTokens ?? 2_000,
    Math.max(250, Math.round(args.promptTokenEstimate * 0.8)),
  );
  ...
  const out =
    hist.sampleCount >= 30 && hist.avgOut
      ? Math.round((fallbackOut + hist.avgOut) / 2)
      : fallbackOut;
  if (hist.sampleCount >= 100 && hist.p50Ms && hist.p99Ms)
    return {
      expectedLatencyMs: Math.round(hist.p50Ms),
      upperLatencyMs: Math.round(hist.p99Ms),
      estimateQuality: "likely",
      latencyPolicy: "p50_p99_tokens_v1",
      latencyBasis: "historical",
      historicalSampleCount: hist.sampleCount,
      expectedInputTokens: input,
      expectedOutputTokens: out,
      expectedTotalTokens: input + out,
      estimatedCostUsd: null,
    };
  if (hist.sampleCount >= 30 && hist.p50Ms && hist.p99Ms)
    return {
      expectedLatencyMs: Math.round((heuristicMs + hist.p50Ms) / 2),
      upperLatencyMs: Math.round((heuristicMs * 2.5 + hist.p99Ms) / 2),
      estimateQuality: "uncertain",
      latencyPolicy: "blend_p50_p99_tokens_v1",
      latencyBasis: "blended",
      historicalSampleCount: hist.sampleCount,
      expectedInputTokens: input,
      expectedOutputTokens: out,
  ...
  return {
    expectedLatencyMs: heuristicMs,
    upperLatencyMs: Math.round(heuristicMs * 2.5),
    estimateQuality: hist.sampleCount > 0 ? "uncertain" : "rough",
    latencyPolicy: "heuristic_tokens_v1",
    latencyBasis: "heuristic",
    historicalSampleCount: hist.sampleCount,
    expectedInputTokens: input,
    expectedOutputTokens: out,
...
export function latencyOutcome(actualMs: number, expectedMs: number, upperMs: number) {
  ...
  return {
    deviationMs,
    deviationPct: expectedMs > 0 ? (deviationMs / expectedMs) * 100 : null,
    result: classifyLatency(actualMs, expectedMs, upperMs),
  };
}
