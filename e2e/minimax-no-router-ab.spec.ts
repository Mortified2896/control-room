...
test.describe("MiniMax chat without router A/B", () => {
  test("selecting MiniMax + routerAb on does not surface unsupported-router-ab error", async ({
    ...
    const minimax = (envPayload.models as Array<{
      providerId: string;
      modelId: string;
      enabled: boolean;
    ...
    await trigger.click();
    await page.getByRole("button", { name: /MiniMax/i }).first().click();
    ...
    await expect(
      page.getByText(/unsupported_router_ab_provider/i),
    ).toHaveCount(0, { timeout: 15_000 });
    await expect(
      page.getByText(/deterministic stub response/i),
