...
launchctl bootout "gui/$(id -u)/com.controlroom.otel-retention" 2>/dev/null || true
PLIST="$HOME/Library/LaunchAgents/com.controlroom.otelcol.plist"; [ ! -f "$PLIST" ] || rm "$PLIST"
RETENTION_PLIST="$HOME/Library/LaunchAgents/com.controlroom.otel-retention.plist"; [ ! -f "$RETENTION_PLIST" ] || rm "$RETENTION_PLIST"
