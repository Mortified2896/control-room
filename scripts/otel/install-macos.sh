...
for d in bin config data/lean/logs data/lean/traces data/lean/metrics data/forensic/traces state collector-logs; do mkdir -p "$OTEL_HOME/$d"; done
...
install -m 0755 "$ROOT/scripts/otel/control_room_otel.py" "$OTEL_HOME/bin/control_room_otel.py"
...
RETENTION_PLIST="$HOME/Library/LaunchAgents/com.controlroom.otel-retention.plist"
HOST=$(hostname -s)
...
sed -e "s|@@HOME@@|$OTEL_HOME|g" "$ROOT/scripts/otel/launchd/com.controlroom.otel-retention.plist.template" > "$RETENTION_PLIST"
plutil -lint "$PLIST" >/dev/null
plutil -lint "$RETENTION_PLIST" >/dev/null
...
launchctl bootout "gui/$(id -u)" "$RETENTION_PLIST" 2>/dev/null || true
launchctl bootstrap "gui/$(id -u)" "$RETENTION_PLIST"
