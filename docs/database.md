...
| `db/migrations/0006_router_settings.sql`          | Adds the `router_settings` singleton row (id=1 CHECK constraint) that backs the `/settings/router` Settings UI. The full settings payload is stored as JSONB; the chat route reads through `lib/repo/router-settings.ts` and merges with env defaults. |
| `db/migrations/0007_model_discovery.sql`          | Adds `openai_models_discovery` (cached `/v1/models` payload + freshness metadata) and `model_selector_prefs` (per-model show/hide for the manual chat selector). Backs the dynamic model discovery feature in `/settings/router`.                      |
| `db/migrations/0008_model_discovery_previous.sql` | Adds `previous_model_ids` to `openai_models_discovery` so the registry layer can derive `stale` (was in the last refresh but absent from the current one) without keeping a separate history table.                                                    |
...
- `lib/repo/router-settings.ts` — Router Settings singleton row
  (`router_settings`). Read path returns `RouterSettings | null`; write path
  upserts through the strict `parseRouterSettingsForSave` validator.
